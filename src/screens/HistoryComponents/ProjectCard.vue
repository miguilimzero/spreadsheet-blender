...
export default {
	...
	methods: {
		...
		async recoverProject(historyHash) {
			...
			// Block application to load everything
			await this.loadProjectTables()
		...
		async loadProjectTables() {
			await this.$root.project.spreadsheetList.forEach(async file => {
				this.$root.spreadsheetData[file.path] = await this.$root.getSpreadsheetData(file)
			});
		}
	},
}
